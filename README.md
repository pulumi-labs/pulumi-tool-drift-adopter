  ...
  "resources": [
    {
      ...
      "properties": [
        ...
        {
          "path": "vpcId",
          "currentValue": "vpc-old123",
          "desiredValue": "vpc-abc123",
          "dependsOn": {
            "resourceName": "my-vpc",
    ...
    },
    {
      ...
      "properties": [
        {
          "path": "ami",
          "desiredValue": "ami-0abcdef1234567890"
        },
        {
          "path": "instanceType",
          "desiredValue": "t3.micro"
        },
        {
          "path": "subnetId",
          "dependsOn": {
...
**Actions:** `update_code` (update properties to match `desiredValue`), `delete_from_code` (remove resource from code), `add_to_code` (add resource to code). All action types use a unified `properties` array of `PropertyChange` objects with leaf-level `path` (e.g., `tags.Environment`, `subnetIds[0]`), optional `currentValue`/`desiredValue`, and optional `dependsOn`.

**`dependsOn`:** When a property references another resource's output, `dependsOn` is set with `resourceName`, `resourceType`, and optionally `outputProperty`. When present, `desiredValue` is omitted — the agent should use the referenced resource's output. Applies to both `update_code` and `add_to_code`.
